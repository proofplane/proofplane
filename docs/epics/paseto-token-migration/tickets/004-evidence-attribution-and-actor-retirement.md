...
- [x] Given a valid PASETO bearer token, when it calls an allowed data-plane route after cutover, then the request runs as that user and API token.
- [x] Given a PASETO-authenticated evidence submission, when it is created, then only its API-token foreign key is stored; when it is read, then the submitting user is derived through that token.
...
- [x] Wire every REST data-plane route to the shared PASETO verifier and preserve permission/workspace non-disclosure behavior.
...
- [x] Remove `api-keys-simplified` and replace actor-based local/integration seeds with users and PASETO tokens.
