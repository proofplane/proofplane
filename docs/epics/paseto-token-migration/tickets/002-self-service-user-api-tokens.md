...
their own workspace-scoped opaque API tokens with explicit permissions and
...
- [x] Given a current workspace member, when they issue a token with valid permissions and a future expiration, then the response shows the raw opaque token once as `api_token` and stores only its digest and lifecycle metadata.
...
- [x] Mint opaque `ppat_` credentials with required digest persistence, workspace, permissions, and expiration without a maximum TTL.
...
- The 2026-06-19 spec revision preserves this management and lifecycle model
  while replacing PASETO issuance with compact opaque issuance and digest
  persistence.
