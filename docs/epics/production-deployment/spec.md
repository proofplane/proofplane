...
raise it. `verify-full` pairs a mandatory handshake with an `openssl`
...
The connector is OpenSSL on every platform, through `postgres-openssl`.
`native-tls` was the first choice and is the reason the field pair exists in
this shape, but it delegates to Security.framework on macOS. Apple refuses a
server certificate that is valid for more than 398 days, and Supabase issues
one valid for 1825 days, so a developer on macOS could not reach an endpoint
that the release image verifies without complaint. One stack on every platform
removes that divergence, and it removes the need to parse a root certificate
through a second library.
...
through `X509StoreBuilderRef::add_cert`. The built-in roots are never disabled,
so one deployment may hold an endpoint on a public root and an endpoint on a
private one. Supabase issues a private root, which is why the field exists.
...
emptying a variable, and an empty value is refused as malformed. The certificate
needs no `_FILE` twin, because unlike the URL it is public.
...
- 2026-08-17: Rollback after a migration is supported again, reversing the
  roll-forward-only recovery #157 introduced. The exact-match check contradicted
  the expand-then-contract rule it sits next to: that rule exists to buy N-1
  compatibility against a forward schema, and the check then refused to use it.
  The stated mitigation did not hold either, because API and MCP run at min
  instances zero, so an old revision kept for diagnosis fails the check on its
  next cold start. The prescribed recovery was in practice the previous release
  commit with the new migration cherry-picked onto it, an untested artifact
