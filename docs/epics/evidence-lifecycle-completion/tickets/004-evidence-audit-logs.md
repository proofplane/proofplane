...
acceptance, download-grant issuance/use, scan outcomes, and finalization.

**Acceptance criteria**

- [ ] Given a successful submission, upload acceptance, grant issuance,
  grant download, scan terminal outcome, or finalization, when it completes, then
  one attributable
...
- [ ] Given attachment activity, when audit fields are inspected, then metadata
  identifiers, grant ID, and outcomes may appear but raw grant tokens, file
  bytes, credentials, and scanner error strings do not.
...
- [ ] Instrument submission/upload/grant service outcomes.
