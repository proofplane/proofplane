...
- [x] Given the selected filename already exists, when the upload is accepted,
  then the signed UI flow stores a macOS-style suffixed filename instead of
  failing on the duplicate.
- [x] Given upload succeeds, when the page updates, then it says "Uploaded" and
...
- [x] Add server-side duplicate filename suffixing for the signed browser flow.
- [x] Render existing attachment inventory and post-upload stored filename.
- [x] Add integration tests for page rendering, upload success, duplicate
