...
document acceptance, download grant issuance and redemption, scan terminal
...
- [ ] Given audit fields are inspected, then workspace, user, agent connection,
  ...
  ID, and document ID may appear where applicable. (Actor is the agent
...
- [ ] Given sensitive values exist during the flow, then raw grant tokens, API
  tokens, authorization headers, document bytes, object keys treated as
...
- [ ] Define stable evidence audit event names and allowed fields in the
  Reliability spec.
- [ ] Instrument submission creation and document acceptance after successful
  commits.
