...
- Submission scan/finalization tests already cover concrete Postgres rollback,
...
- initial quarantine-write failure in the submission upload API: return a stable
  error and commit no submission row or scan-request outbox event;
- final-object read failure in the human download route: return a stable error
  without changing the persisted submission lifecycle;
- worker finalization copy failure: return a retryable delivery error and leave
  the submission `finalizing`;
- database failure after a successful finalization copy: retry
  `mark_submission_uploaded` within the handler using the shared `Retryable`
...
- worker quarantine-delete failure after a successful copy: keep the submission
...
credentials or submission bytes.
...
submission row. The worker later owns the copy from quarantine to the final
object key. Submission lifecycle state, exposed to agents through the normal
...
does not mark the submission `failed`, because finalized bytes may already
...
submission remains `finalizing` for operational recovery.
...
Never label with workspace, actor, request, object, submission, submission,
...
hashes, bearer grant tokens or URLs, internal object keys, submission or packet
...
submission ID, submission ID, grant ID, and coarse lifecycle status where
...
tokens, access tokens, authorization headers, submission bytes, storage object
keys treated as internals, scanner raw error strings, credentials, or unbounded
dependency error strings.

Submission creation, submission acceptance, and download-grant issuance success
...
submission remains eligible for streaming. Scan and finalization terminal
