...
connection definition. Clients with compatible OAuth discovery and Proofplane
registration behavior can use the hosted OAuth path; unsupported clients stay
on advanced API-token setup.

**Acceptance criteria**

- [ ] Given a generic MCP client without a supported Proofplane registration path,
