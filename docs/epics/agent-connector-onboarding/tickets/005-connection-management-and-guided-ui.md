...
- [ ] Given a valid Proofplane OAuth transaction, when the user opens the
  ...
  and the user's single workspace as a fixed approval (no picker).
...
- [ ] Given an owned connection, when it is revoked, then it is marked revoked
  locally before remote Auth0 cleanup and unrelated connections are unchanged.
...
- [ ] Add authorization status and first-prompt content without mirroring
...
- 2026-07-09: `ppat_` API tokens were removed in PR #42, so this ticket no
  ...
  The consent step shows the user's single workspace as a fixed approval, not a
  picker.
