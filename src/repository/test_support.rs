//! Postgres fixtures for repository tests.
//!
...
use crate::{
    domain::{AgentConnectionId, EvidenceId, PolicyId, UserId, WorkspaceId},
    store,
};
...
fn shared() -> &'static SharedContainer {
    SHARED.get_or_init(|| {
        ...
        std::thread::spawn(|| {
            let runtime = Runtime::new().expect("repository test runtime builds");
            let (container, host, port) = runtime.block_on(async {
                ...
                let admin = store::conn(&admin_url(&host, port))
                    .await
...
pub async fn database() -> Postgres {
    ...
    let admin = store::conn(&shared.url("postgres"))
        .await
        .expect("administrative database connection opens");
    admin
        .execute(
    ...
    let pool = store::conn_pool(&shared.url(&name), 4)
        .await
