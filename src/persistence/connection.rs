...
pub enum Error {
    ...
    RootCertificatePem(#[source] ErrorStack),

    #[error("failed to trust the configured database root certificate")]
    RootCertificate(#[source] ErrorStack),
...
enum Transport {
    Plaintext,
    /// A connector that verifies the certificate chain and the hostname
    /// against the system certificate store and any root the configuration
    /// adds.
    ...
    fn build(tls: &DatabaseTlsConfig) -> Result<Self, Error> {
        match tls.mode {
            ...
            DatabaseTls::VerifyFull => {
                // This loads the system certificate store, requires a peer
                // certificate, and verifies the hostname.
                let mut builder = SslConnector::builder(SslMethod::tls()).map_err(Error::Tls)?;

                if let Some(pem) = &tls.root_certificate {
                    let roots =
                        X509::stack_from_pem(pem.as_bytes()).map_err(Error::RootCertificatePem)?;
                    ...
                    for certificate in roots {
                        builder
                            ...
                            .map_err(Error::RootCertificate)?;
                    }
                }

                Ok(Self::Verified(MakeTlsConnector::new(builder.build())))
            }
        }
    }
