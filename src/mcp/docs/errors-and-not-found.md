...
- Conflict codes such as `control_code_taken` and `evidence_control_mapping_exists` mean the requested state already conflicts with stored state. Read the object or mapping, then choose whether to use it, replace it, or stop.
...
Do not retry validation or conflict failures unchanged. Retry transient dependency failures, and request a new upload URL when only that short-lived grant has expired.
