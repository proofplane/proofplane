...
- **Document**: a file associated with a submission. A human transfers file bytes through a short-lived browser flow; bytes never pass through MCP or the model.
...
Auditor access is separate from document handling. An auditor access grant gives the named auditor a bearer-secret browser link to review evidence until expiry or revocation.
