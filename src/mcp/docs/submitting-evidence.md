...
4. Call `list_evidence_submissions` to confirm what arrived, and inspect each submission's filename, checksums, size, content type, and `upload_status`. Do not assume that issuing a URL means the upload succeeded. Use `get_evidence_submission` when you already hold a submission ID.
