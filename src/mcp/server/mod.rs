...
const SERVER_INSTRUCTION_LEAD: &str = concat!(
    ...
    "list_evidence and read its collection_instructions; second, call ",
    "manage_evidence_submissions with the evidence ID and the coverage window the proof covers ",
    "to get a short-lived human browser flow; third, a human uploads files there and each file ",
    "becomes one submission for that window. File bytes never pass through MCP or the model. ",
    "Confirm results with list_evidence_submissions. "
);

const SERVER_INSTRUCTION_DETAIL: &str = concat!(
    "Frameworks contain requirements, requirements are ",
    "satisfied by controls, and control mappings link controls to evidence. A submission is one ",
    "file with a coverage window and the time it was received. Several submissions may share a ",
    "coverage window when one file cannot cover the period. To replace proof, archive a ",
    "submission and upload another. Controls define what must be proven, so review their ",
    "mappings when deciding which proof satisfies evidence. Submissions record the connected ",
    "agent's provenance. Treat the browser URL as a bearer secret and share it only with the ",
    "human uploading the files before it expires. Call get_proofplane_guide without a topic to ",
    "see its topic index. Clients that surface MCP resources can also browse these guides at ",
...
impl ProofplaneMcp {
    ...
    fn tool_router() -> ToolRouter<Self> {
        ToolRouter::new()
            ...
            + Self::evidence_upload_grants_tool_router()
...
mod tests {
    ...
    fn instructions_cover_relationships_and_operational_constraints() {
        ...
        for expected in [
            ...
            "A submission is one file with a coverage window",
            "Several submissions may share a coverage window",
            "archive a submission and upload another",
