...
impl ProofplaneMcp {
    #[tool(
        ...
        description = "Create a piece of evidence that states what the organization must prove and how to collect it; for guidance, call get_proofplane_guide with topic submitting-evidence."
    )]
    ...
    #[tool(
        name = "list_evidence",
        description = "List evidence with their collection instructions and status; for guidance, call get_proofplane_guide with topic submitting-evidence."
    )]
...
#[cfg(test)]
mod tests {
    use super::{parse_create_evidence, CreateEvidence};
    use crate::domain::EvidenceStatus;

    #[test]
    fn create_evidence_parses_payload() {
        let payload = parse_create_evidence(CreateEvidence {
