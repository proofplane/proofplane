...
/// Uploads one file as a new evidence submission for `evidence_id` over the given
/// coverage window, mirroring the browser flow where each file becomes one
/// submission plus its single `Document`. Returns the submission id and the
/// document metadata.
pub async fn submit_evidence_file(
    app: &TestApp,
    workspace_id: Uuid,
    evidence_id: Uuid,
    valid_from: &str,
    ...
    let coverage = CoverageWindow::new(
        valid_from.parse().expect("valid_from parses"),
        valid_until.parse().expect("valid_until parses"),
    )
    ...
    let document = service
        ...
        .expect("evidence exists for submission");
...
impl TestApp {
    ...
    /// Inserts a piece of evidence directly, bypassing the API, so tests can seed
...
"#,
    ...
    pub fn document_upload_grant_service(&self) -> DocumentUploadGrantService {
        ...
        DocumentUploadGrantService::new(
            ...
            UploadGrantDecryptor::from_config(
                issuer,
                "proofplane-document-upload-grant",
                &self.app_config.paseto.upload_grant,
            )
    ...
    pub fn document_download_service(&self) -> DocumentDownloadService {
        ...
        DocumentDownloadService::new(
            ...
            DownloadGrantDecryptor::from_config(
                issuer,
                "proofplane-document-download",
                &self.app_config.paseto.download,
            )
